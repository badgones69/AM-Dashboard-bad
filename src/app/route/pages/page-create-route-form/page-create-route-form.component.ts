...
export class PageCreateRouteFormComponent implements OnInit {
  ...
  public existingAirports: Airport[] = [];
  // List of already existing routes
  public existingRoutes: Route[] = [];
  ...
  ngOnInit(): void {
    ...
    this.loadExistingAirportsAndRoutes();
  ...
  // Existing airports and routes load
  private loadExistingAirportsAndRoutes() {
    this.routeService.getRoutes().subscribe(routes => this.existingRoutes = routes);

    this.airportService.getAirports().subscribe(airports => {
      this.departureHubs = airports.filter(a => a.hub);
      this.existingAirports = airports;
  ...
  private configureArrivalAirportFieldAutocomplete(departureHub: Airport) {
    ...
    this.arrivalAirportsAvailable = this.existingAirports.filter(airport => airport.id != departureHub.id);
  ...
  private configureCountryFieldAutocomplete() {
    this.countryService.getCountries().subscribe(countriesAvailable => {
      this.countries = countriesAvailable;
  ...
  addDepartureHubListener(departureHubValueChanged: any) {
    if (departureHubValueChanged != null) {
      ...
      if (departureHubFound != undefined) {
        ...
        this.routeForm.value.departureHub = departureHubFound;
    ...
    this.arrivalAirportSelected.valueChanges.subscribe(arrivalAirportValueChanged => {
      if (arrivalAirportValueChanged != null) {
        ...
        if (arrivalAirportFound != undefined) {
          ...
          this.routeForm.value.arrivalAirport = arrivalAirportFound;
        } else {
          this.setDefaultArrivalAirport();
        }
      } else {
  ...
  addCountryListener(countryValueChanged: any) {
    if (countryValueChanged != null) {
      ...
      if (countryFound != undefined) {
        ...
        this.routeForm.value.country = countryFound;
        this.setDefaultRegion();
    ...
    this.newArrivalAirportRegion.valueChanges.subscribe(regionValueChanged => {
      if (regionValueChanged != null) {
        ...
        if (regionFound != undefined) {
          ...
          this.routeForm.value.region = regionFound;
        } else {
          this.flagNewArrivalAirportRegion = EMPTY_REGION_FLAG;
        }
      } else {
      ...
      if (this.isValidDepartureHub() && this.isValidIATA() && this.isValidCity() && this.isValidCountry() && this.isValidRegion()) {
        ...
        this.airportService.createAirport(arrivalAirport).subscribe(arrivalAirportCreated => {
          if (arrivalAirportCreated.id != null) {
            this.createNewRoute(this.routeForm.value.departureHub, arrivalAirportCreated);
          }
        });
      } else {
  ...
  // Route creation
  private createNewRoute(departureHub: Airport, arrivalAirport: Airport) {
    this.routeService.createRoute(departureHub, arrivalAirport).subscribe(routeCreated => {
  ...
  isValidArrivalAirport() {
    let existingRoutesArrivalsForDepartureHub: any[] = this.existingRoutes.filter(route => route.departureHub.id === this.routeForm.value.departureHub.id).map(route => route.arrivalAirport.id);
  ...
  isValidIATA() {
    let alreadyExistingAirportsIATA: string[] = this.existingAirports.map(a => a.iata);
